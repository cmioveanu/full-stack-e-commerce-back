...
account.post('/register', (req, res) => {
    ...
    WHERE email = $1`, [username], (err, result) => {
        if (err) {
            console.error('Unable to get customers from DB', err);
        }
        ...
        if (result.rows[0]) {
            ...
            bcrypt.hash(password, 5, (err, hash) => {
                ...
                VALUES ($1, $2, $3, $4, $5)`, [username, hash, firstName, lastName, phone], (err, result) => {
                    if (err) {
                        console.error('Unable to register customer', err);
                    }
...
//change email
account.put('/email', checkAuth, (req, res) => {
    ...
    WHERE id = $1`, [req.user.id], (err, result) => {
        if (err) {
            console.error('Unable to get customer password from DB', err);
        }
...
//change password
account.put('/password', checkAuth, (req, res) => {
    ...
    pool.query(`
        ...
        if (err) {
            console.error('Unable to select password', err);
        }
        ...
        bcrypt.compare(oldPassword, DBPassword, (err, check) => {
            if (err) {
                console.error('Password don\'t match', err);
            }
            ...
            if (check) {
                bcrypt.hash(newPassword, 5, (err, hash) => {
                    ...
                    `, [hash, req.user.id], (err, result) => {
                        if (err) {
                            console.error('Unable to update customer with new password', err);
                        }
